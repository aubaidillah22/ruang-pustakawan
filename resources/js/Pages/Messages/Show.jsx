...
export default function MessagesShow({ conversationUser, messages: initialMessages, isOnline }) {
    ...
    // Keyboard + body scroll for lightbox
    useEffect(() => {
        if (!lightboxImg) {
    ...
    return (
        <AuthenticatedLayout>
            ...
            {/* Enhanced Image Lightbox - no gap layout */}
            {lightboxImg && (
                <div className="lightbox-overlay fixed inset-0 z-[100] flex flex-col"
                     style={{ background: 'rgba(0,0,0,0.92)' }}
                     onClick={() => setLightboxImg(null)}>

                    {/* Glass Header */}
                    <div className="lightbox-header lightbox-glass flex-shrink-0 flex items-center justify-between px-5 py-4">
                    ...
                    {/* Media with zoom - fills remaining space */}
                    <div className="lightbox-media flex-1 relative min-h-0 select-none overflow-hidden"
                         onClick={(e) => e.stopPropagation()}>
                        <img
                            src={lightboxImg}
                            alt="Gambar"
                            className="max-w-full max-h-full w-full h-full object-contain cursor-pointer"
                            draggable={false}
                            ...
                            onDoubleClick={(e) => {
                                ...
                                if (currentScale === '1') {
                                    ...
                                    img.style.transform = `scale(2.5) translate(${-x*200}px, ${-y*200}px)`;
                                } else {
                                    img.dataset.scale = '1';
